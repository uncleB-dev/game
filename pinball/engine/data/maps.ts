...
/* ──────────────────────────────────────────────────────────
   UncleB Games 오리지널 맵 (생성 함수로 정의)
   ────────────────────────────────────────────────────────── */

const WALL_PROPS = { density: 1, angularVelocity: 0, restitution: 0 };
...
function pin(x: number, y: number, radius = 0.22, restitution = 0.45): MapEntity {
  return {
    type: 'static',
    position: { x, y },
...
function bubble(x: number, y: number, radius = 0.45): MapEntity {
  return {
    type: 'static',
    position: { x, y },
    props: { density: 1, angularVelocity: 0, restitution: 1.2, life: 1 },
    shape: { type: 'circle', radius },
  };
}

function spinner(x: number, y: number, width: number, av: number, height = 0.15): MapEntity {
  return {
    type: 'kinematic',
    position: { x, y },
    props: { density: 1, angularVelocity: av, restitution: 0 },
    shape: { type: 'box', width, height, rotation: 0 },
  };
}

function shelf(x: number, y: number, width: number, rotation: number): MapEntity {
  return {
    type: 'static',
    position: { x, y },
    props: { ...WALL_PROPS },
    shape: { type: 'box', width, height: 0.2, rotation },
  };
}

/** 맵 5: 플링코 폭포 — 핀 숲을 뚫고 떨어지는 클래식 플링코 + 회전 바 */
function buildPlinkoFalls(): StageDef {
  const entities: MapEntity[] = [
    wall([[9.25, -300], [9.25, 2], [3, 12], [3, 58], [10.5, 68], [10.5, 86]]),
    wall([[16.5, -300], [16.5, 2], [22.75, 12], [22.75, 58], [15.25, 68], [15.25, 86]]),
  ];

  // w 는 반폭(half-width) — planck Box 컨벤션
  const spinners = [
    { x: 12.875, y: 33, w: 2, av: 2.2 },
    { x: 7.5, y: 45, w: 1.7, av: -2.8 },
    { x: 18.25, y: 45, w: 1.7, av: 2.8 },
    { x: 12.875, y: 63, w: 1.3, av: 3 },
  ];

  // 지그재그로 어긋난 핀 숲 (스피너 주변은 비움)
  for (let row = 0; row < 11; row++) {
    const y = 16 + row * 3.8;
    const startX = row % 2 === 0 ? 4.4 : 5.5;
    for (let x = startX; x <= 21.6; x += 2.2) {
      const nearSpinner = spinners.some((s) => Math.hypot(x - s.x, y - s.y) < s.w + 1.2);
      if (nearSpinner) continue;
      entities.push(pin(x, y));
    }
  }

  for (const s of spinners) entities.push(spinner(s.x, s.y, s.w, s.av));

  // 바닥 디플렉터 — 마지막 갈림길
  entities.push(shelf(6, 62, 1.5, 0.6));
  entities.push(shelf(19.75, 62, 1.5, -0.6));

  return { title: '플링코 폭포', goalY: 84, zoomY: 80, entities };
}

/** 맵 6: 번개 지그재그 — 좌우로 꺾이는 급류 + 범퍼 + 버블 게이트 */
function buildZigzagRush(): StageDef {
  const entities: MapEntity[] = [
    wall([[9.25, -300], [9.25, 2], [2, 10], [2, 74], [10, 84], [10, 96]]),
    wall([[16.5, -300], [16.5, 2], [24, 10], [24, 74], [16, 84], [16, 96]]),
  ];

  // 번갈아 기운 선반 8개 — 갭 반대편으로 흐른다
  for (let i = 0; i < 8; i++) {
    const y = 14 + i * 7.5;
    if (i % 2 === 0) {
      entities.push(shelf(9.2, y, 7.2, 0.28)); // 왼쪽 부착(2~16.4), 우측 갭으로 우하향
      entities.push(pin(20.2, y + 3.6, 0.55, 1.3)); // 갭 아래 범퍼
    } else {
      entities.push(shelf(16.8, y, 7.2, -0.28)); // 오른쪽 부착(9.6~24), 좌측 갭으로 좌하향
      entities.push(pin(5.8, y + 3.6, 0.55, 1.3));
    }
  }

  entities.push(spinner(13, 40.8, 1.6, -2.6));

  // 골 진입부 버블 게이트 — 터뜨리며 통과
  for (let x = 10.8; x <= 15.3; x += 1.1) entities.push(bubble(x, 78));
  for (let x = 10.3; x <= 15.8; x += 1.08) entities.push(bubble(x, 79.8));

  return { title: '번개 지그재그', goalY: 94, zoomY: 90, entities };
}

stages.push(buildPlinkoFalls(), buildZigzagRush());
